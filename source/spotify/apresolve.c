...
struct in_addr * lookup_host(const char* host)
{
    ...
    {
        log_error("[APRESOLVE] ERR lookup_host: getaddrinfo failed with error %d\n", errcode);
        return NULL;
    }
...
struct sockaddr_in* apresolve()
{
    ...
    if (addr == NULL) {
        log_error("[APRESOLVE] Could not resolve apresolve.spotify.com");
        goto cleanup;
    }
    ...
    if (socket_desc == -1)
    {
        log_error("[APRESOLVE] Could not create socket\n");
        goto cleanup;
    }
    ...
    {
        log_error("[APRESOLVE] Failed to receive HTTP response\n");
        goto cleanup;
    }
    ...
    if (body == NULL) {
        log_error("[APRESOLVE] Malformed HTTP response\n");
        goto cleanup;
    }
    ...
    if (json == NULL) {
        log_error("[APRESOLVE] Malformed JSON\n%s\n", body);
        goto cleanup;
    }

    if (!cJSON_HasObjectItem(json, "ap_list")) {
        log_error("[APRESOLVE] JSON doesn't have ap_list key :(\n%s\n", body);
        goto cleanup;
    }
    ...
    if (cJSON_GetArraySize(ap_list) == 0) {
        log_error("[APRESOLVE] No APs available :(\n%s\n", body);
        goto cleanup;
    }
    ...
    if (!cJSON_IsString(ap_first)) {
        log_error("[APRESOLVE] First AP is not a string :(\n%s\n", body);
        goto cleanup;
    }
    ...
    if (ap == NULL) {
        log_error("[APRESOLVE] First AP seems like a string, but has nullptr value :(\n%s\n", body);
        goto cleanup;
    }
    ...
    if (colon == NULL)  {
        log_error("[APRESOLVE] First AP doesn't specify port :(\n%s\n", body);
        goto cleanup;
    }
    ...
    log_debug("[APRESOLVE] Hostname: %s Port: %s ", hostname, colon + 1);
