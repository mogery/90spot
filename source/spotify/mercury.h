...
struct mercury_response_part {
    uint8_t* buf;
    size_t len;

    struct mercury_response_part* next;
};
...
typedef int (*mercury_response_handler)(
    ...
    struct mercury_response_part* parts, // The parts of the rest of the message. Handler should be prepared for this to be NULL.
...
struct mercury_pending_message {
    ...
    struct mercury_response_part* parts;
...
typedef struct mercury_response_part mercury_response_part;
